...
const AbsoulteDecompPlot = () => {
  const chartData = Object.entries(
    data['Philadelphia Cream Cheese']['dairy']
  ).map(ggvar => ({
    id: ggvar[0],
    data: zip(ggvar[1]['Absolute_Decomp'], ggvar[1]['Effect_on_PQ']).map(
      item => ({
        x: item[0],
        y: item[1],
        symbol: ggvar[1].symbol[0],
        point:
          item[0] == ggvar[1]['current_Absolute_Decomp'][0] &&
          item[1] == ggvar[1]['current_Effect_on_PQ'][0],
        bigPoint:
          item[0] == ggvar[1]['current_Opt_Absolute_Decomp'][0] &&
  ...
  return (
    ...
    <ResponsiveLine
      ...
      axisBottom={{
        ...
        tickPadding: 5,
        tickRotation: 0,
        legend: 'Signature Absolute Decomp',
        legendOffset: 36,
...
export default AbsoulteDecompPlot;
